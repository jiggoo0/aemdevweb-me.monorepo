...
const fetchIdentities = async (): Promise<Identity[]> => {
  const registryUrl = process.env.UNLINK_REGISTRY_URL;
  const apiKey = process.env.REGISTRY_API_KEY;

  if (!registryUrl || !apiKey) {
    throw new Error("Registry configuration missing (URL or API Key)");
  }

  const response = await fetch(`${registryUrl}/api/identities`, {
    headers: {
...
export const syncRegistryToAudit = async (actorId: string = "SYSTEM-AUDIT") => {
  ...
  const coreIdentities = await fetchIdentities();
  const results = [];
